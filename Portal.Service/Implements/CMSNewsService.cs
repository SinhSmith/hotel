...
namespace Portal.Service.Implements
{
    public class CMSNewsService : ICMSNewsService
    {
        public IList<CMSNewsView> GetCMSNews(int pageNumber, int pageSize, out int totalItems)
        {
            ...
            {
                totalItems = db.cms_News.Count(x => x.Status != (int)Portal.Infractructure.Utility.Define.Status.Delete);

                return db.cms_News.Where(x => x.Status != (int)Portal.Infractructure.Utility.Define.Status.Delete)
        ...
        private IList<CMSNewsView> GetCMSNewsRecursive(int categoryId)
        {
            using (var db = new PortalEntities())
            {
                ...
                {
                    news.AddRange(db.cms_News.Where(x => x.CategoryId == category.Id && x.Status == (int)Portal.Infractructure.Utility.Define.Status.Active)
                        .Select(x => new CMSNewsView
                    {
                        Id = x.Id,
                        CategoryId = x.CategoryId,
        ...
        public IList<CMSNewsView> GetCMSNewsByCategoryId(int categoryId, int pageNumber, int pageSize, out int totalItems)
        {
            using (var db = new PortalEntities())
            {
                var news = db.cms_News.Where(x => x.CategoryId == categoryId && x.Status == (int)Portal.Infractructure.Utility.Define.Status.Active)
                    .Select(x => new CMSNewsView
                    {
                        Id = x.Id,
                        CategoryId = x.CategoryId,
        ...
        public IList<CMSNewsView> GetRecentCMSNews()
        {
            using (var db = new PortalEntities())
            {
                return db.cms_News.Where(x => x.Status == (int)Portal.Infractructure.Utility.Define.Status.Active)
        ...
        public IList<CMSNewsView> GetRelatedCMSNews(int id)
        {
            using (var db = new PortalEntities())
            {
                ...
                return db.cms_News.Where(x => x.Status == (int)Portal.Infractructure.Utility.Define.Status.Active && x.CategoryId == news.CategoryId && x.Id != news.Id)
                    .OrderByDescending(x => x.CreatedDate)
                    .Take(3)
                    .Select(x => new CMSNewsView
                    {
        ...
        public IList<CMSNewsView> GetCMSNewsForHomePage()
        {
            using (var db = new PortalEntities())
            {
                return db.cms_News.Where(x => x.Status == (int)Portal.Infractructure.Utility.Define.Status.Active && x.DisplayHomePage == true)
                    .OrderByDescending(x => x.CreatedDate)
                    .Take(3)
                    .Select(x => new CMSNewsView
                    {
